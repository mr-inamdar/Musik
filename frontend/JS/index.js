...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (localStorage.getItem('token')) {
        ...
        if (media_query_for_mobile.matches && allPlaylistSongs.length > 0) {
            renderPlaylist();
        }
...
function renderSongs() {
    ...
    if (trendingSongsC) {
        ...
        Albums.forEach(album => {
            ...
            // const songPlay = songItem.querySelector('.album_song_play') || null;
            // if (album_note && closebtn && songPlay) {
            //     album_note.addEventListener("click", () => activeSongList(album_note, closebtn, songPlay));
            // }
...
function renderPlaylist() {
    ...
    if (!playlist_songs) return;

    playlist_songs.innerHTML = '';

    allPlaylistSongs.forEach((song, i) => {

        const playlistSongli = document.createElement('li');

        playlistSongli.innerHTML = renderPlaylistSong(song, i);

        const playIcon = playlistSongli.querySelector('.playListPlay');
        ...
        if (playIcon) {
            playIcon.addEventListener('click', (e) => {
                ...
                sub_main.style.height =
                    media_query_for_mobile.matches ? '67%' : '75%';

                isPlaylist_song = true;
                playlist_indexing = clickedIndex;

                if (e.currentTarget.classList.contains('bi-pause-circle-fill')) {
                    ...
                    e.currentTarget.classList.remove('bi-play-circle-fill');
                    e.currentTarget.classList.add('bi-pause-circle-fill');
...
function activeSongList(note, closeBtn, songPLay) {

    // Album open / close
    if (note) {

        const albumItem = note.closest(".album_item");

        document
            .querySelectorAll(".album_item.show_songs")
            .forEach(item => {

                if (item !== albumItem) {
                    item.classList.remove("show_songs");
                }

            });

        albumItem.classList.toggle("show_songs");
    ...
    if (closeBtn) {
        ...
        const albumItem = closeBtn.closest(".album_item");

        if (albumItem) {
            albumItem.classList.remove("show_songs");
        }
    ...
    if (songPLay) {
        ...
        const songId = songPLay.dataset.songId;

        const song = allSongs.find(
            song => Number(song.song_id) === Number(songId)
        );

        if (!song) {
            console.log("Song nahi mila:", songId);
            return;
        }
...
document.addEventListener("click", (e) => {
    ...
    const closeBtn = e.target.closest(".close_album_songs");
    const songPLay = e.target.closest(".album_song_play");

    activeSongList(note, closeBtn, songPLay);
...
function loadAndPlaySong(song) {
    ...
    if (dowload_music) {
        ...
        const downloadUrl = audioSrc.replace(
            "/upload/",
            `/upload/fl_attachment:${encodeURIComponent(songTitle)}/`
        );

        dowload_music.href = downloadUrl;
        dowload_music.setAttribute("download", fileName);
    }
    ...
    if (isPlaylist_song) {
        ...
        const playlist_play_icon = document.getElementById(`${playlist_indexing}`);
...
const call_play_next_song = () => {
    if (isPlaylist_song) {
        ...
        // random_index = allSongs.findIndex(song => song.song_id = songId)
    }
    play_next_song();
};
...
function play_music(song) {
    random_index = allSongs.findIndex(s => s.song_id === song.song_id);
    play_system.classList.add('active');
...
const uploadBy = document.querySelector('#uploadBy span');
...
function activate_song_popup(song) {
    ...
    media_query_for_mobile.matches? popupBox.style.width = '90vw':popupBox.style.width = '70vw';
...
function handleHashChange() {
    ...
    if (isMobile) {
        ...
        // Desktop par mobile hash hata do
        if (location.hash) {
...
document
    .getElementById("ur_library")
    ?.addEventListener("click", display_playlist_page);

document
    .getElementById("home")
    ?.addEventListener("click", display_home_page);
...
function handleAlbumUpload(){
    ...
    window.location = 'uploadAlbumForm.html'
}
function handleSongUpload(){
    ...
    window.location = 'uploadSongForm.html'
}
...
addSongBtn.addEventListener("click", function (e) {
    ...
    addSongBtn.innerHTML = `
                <div id="uploadMenu">
                    ...
                    <div id="addSongHeadder">
                        <h6>Upload</h6>
                        <i class="bi bi-x-lg close_album_songs"></i>
                    </div>

                    <div class="addSongItem">
                        <i class="bi bi-disc"></i>
                        <span id='uploadAlbum'>Upload Album</span>
                    </div>

                    <div class="addSongItem">
                        <i class="bi bi-music-note-beamed"></i>
                        <span id='uploadong'>Upload Song</span>
                    </div>
    ...
    const uploadSong = document.getElementById("uploadong");
    const closeBtn = document.querySelector('#addSongHeadder .close_album_songs')

    // animation trigger
    requestAnimationFrame(() => { uploadMenu.classList.add("show"); });

    uploadAlbum.addEventListener('click', handleAlbumUpload);
    uploadSong.addEventListener('click', handleSongUpload)
    
    closeBtn.addEventListener('click', (e)=>{
        e.stopPropagation();
        uploadMenu.classList.remove('show');
        addSongBtn.innerHTML = '<i class="bi bi-plus"></i>';
    });
